...
excel = xlrd.open_workbook("exp-bandwidth-seq.xlsx")
...
plt.plot(xs[3], ys[3], marker='', label=labels[3], linewidth=3.5, linestyle='-', color="red")
...
# plt.legend()          #显示各曲线的图例
ax.legend(labels[:3],loc=4, numpoints=1, ncol=5, fontsize=14, borderpad=False, framealpha=0, bbox_to_anchor=(1, 0.95),labelspacing=0.1, columnspacing=0.5)
leg = ax.get_legend()
ltext = leg.get_texts()
plt.setp(ltext, fontsize=14,fontweight='bold')  # 设置图例字体的大小和粗细
fig.tight_layout()
plt.savefig('./exp-bandwidth-seq2.pdf', format='pdf')  # 建议保存为svg格式,再用inkscape转为矢量图emf后插入word中
plt.show()
