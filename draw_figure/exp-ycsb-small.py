...
for i in range(len(xs)):
    ...
    if i % 4 == 3 :
        for j in range(len(xs[i])):
            plt.text(xs[i][j] - 0.5* bar_width, ys[i][j] + 30, (ys_normal[i][j]), fontsize=12)
    if i % 4 == 2:
        for j in range(len(xs[i])):
            plt.text(xs[i][j] - 1.2 * bar_width, ys[i][j] + 10, (ys_normal[i][j]), fontsize=12)
# 画x刻
plt.xticks(tickx, labelx, rotation=0, fontsize=14)
# r=list(range(0,20,2))
# r.insert(0,1)
plt.yticks([0,500,1000,1500,2000,2500,3000],fontsize=12)
plt.xlim(-0.2,7)
# plt.ylim(0, 17)
...
plt.legend((bars[0], bars[4], bars[1], bars[5], bars[2], bars[6], bars[3], bars[7]),
           (labels[0], labels[4], labels[1], labels[5], labels[2], labels[6], labels[3], labels[7]),ncol=2, fontsize=12, borderpad=False,
