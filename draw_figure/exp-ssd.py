...
excel = xlrd.open_workbook("exp-ssd.xlsx")
...
for i in range(len(xs)):
    ...
    if (i % 3 == 2):
        for j in range(len(xs[i])):
            if (j == 5):
                plt.text(xs[i][j] - 0.8 * bar_width, ys[i][j] + 20, ys_normal[i][j], fontsize=12)
            else:
                plt.text(xs[i][j] - 0.8 * bar_width, ys[i][j] + 20, ys_normal[i][j], fontsize=12)
# 画x刻度
...
# plt.xlabel('Thread number',fontsize=14, fontweight='bold')
...
plt.legend((bars[0], bars[3], bars[1], bars[4], bars[2], bars[5]),
           ...
           handletextpad=0.5,loc=4,bbox_to_anchor=(1, 0.65))
leg = plt.gca().get_legend()
ltext = leg.get_texts()
plt.setp(ltext, fontsize=14, fontweight='bold')  # 设置图例字体的大小和粗细

fig.tight_layout()
plt.savefig('./exp-ssd.pdf', format='pdf')
plt.show()
