...
excel = xlrd.open_workbook("bg-bandwidth-128B.xlsx")
...
plt.hlines(2400, 0, 100, linestyles='--', colors="black", label="SSD max bandwidth", linewidth=3.5)
...
plt.ylabel("IO Bandwidth(MB/s)", fontsize=13,fontweight='bold')
plt.xlim(0, 100)  # 设置x轴的范围
plt.ylim(0, 2400)
# ax_sub = ax.twinx()
# ax_sub.set_ylabel('MQPS',fontsize=14,fontweight='bold')
# ax_sub.set_ylim(0,19.500)
plt.xticks(range(0,100,10),range(0,100,10),fontsize=13)
plt.yticks(fontsize=13)
# plt.legend()          #显示各曲线的图例
ax.legend(loc=4, numpoints=1, ncol=1, fontsize=12, borderpad=False, framealpha=1, bbox_to_anchor=(1, 0.4))
leg = ax.get_legend()
ltext = leg.get_texts()
plt.setp(ltext, fontsize=14,fontweight='bold')  # 设置图例字体的大小和粗细
fig.tight_layout()
plt.savefig('./bg-bandwidth-128B.pdf', format='pdf')  # 建议保存为svg格式,再用inkscape转为矢量图emf后插入word中
plt.show()
