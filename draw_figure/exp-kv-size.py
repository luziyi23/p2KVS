...
excel = xlrd.open_workbook("exp-kv-size.xlsx")
...
for i in range(len(xs)):
    ...
    #             plt.text(xs[i][j] - 0.8 * bar_width, ys[i][j] + 10, ys_normal[i][j], fontsize=12)
# 画x刻度
plt.xticks(tickx, labelx, rotation=0, fontsize=12)
# plt.yticks(fontsize=12)
plt.yticks([0, 2, 4, 6, 8, 10, 12, 14, 16], fontsize=12)
plt.xlim(-0.2, 7)
# plt.ylim(0,58)
plt.ylabel('Normalized QPS\n to RocksDB', fontsize=12, fontweight='bold')
...
#            handletextpad=0.5)
plt.legend(fontsize=12, borderpad=False,
           framealpha=1)
...
fig.tight_layout()
plt.savefig('./exp-kv-size.pdf', format='pdf')
plt.show()
