...
labels = ["RocksDB","PebblesDB", "p$^2$KV-4","p$^2$KV-8"]
ys = []
xs = []
max_y = []
avg_y = []
...
# plt.legend()          #显示各曲线的图例
# ax.legend(loc=4, numpoints=1, ncol=4, fontsize=12, borderpad=False, framealpha=0, bbox_to_anchor=(1, 0.98))
# leg = ax.get_legend()
# ltext = leg.get_texts()
# plt.setp(ltext, fontsize=12,fontweight='bold')  # 设置图例字体的大小和粗细
fig.tight_layout()
plt.savefig('./exp-bandwidth-rand.pdf', format='pdf')  # 建议保存为svg格式,再用inkscape转为矢量图emf后插入word中
plt.show()
