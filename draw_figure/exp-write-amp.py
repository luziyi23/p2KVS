...
excel = xlrd.open_workbook("exp-write-amp.xlsx")
...
for i in range(len(xs)):
    ...
    for j in range(len(xs[i])):
        plt.text(xs[i][j] - 0.5 * bar_width, ys[i][j] + 0.5, str(ys[i][j])+"$\\times$", fontsize=11)
# 画x刻度
plt.xticks(tickx, labelx, rotation=0, fontsize=14)
plt.yticks(fontsize=14)
plt.xlim(-0.2,1.8)
plt.ylim(0,20)
plt.ylabel('IO Amplification', fontsize=12, fontweight='bold')
...
plt.legend((bars[2],bars[3]),(labels[2],labels[3]),ncol=4, fontsize=14, borderpad=False, framealpha=0,loc=4,bbox_to_anchor=(0.9, 1))
...
fig.tight_layout()
plt.savefig('./exp-write-amp.pdf', format='pdf')
plt.show()
